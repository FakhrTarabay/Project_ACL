...
//This route is to update the constructor
            ...
            if(departCourses.includes(course.name)){
              //try{
                 if(courseInstructors.includes(Assignedmemberstaff.id)){
                           res.send("You are trying to update with already assigned constructor") 
                }
                  else{
  ...
  if(role_of_memberstaff=="HOD"){
     ...
     try{
        const staff = await staffMemberModel.find({department:memberstaff.department},'id name email dayOff')
        res.send(staff)
...
.post(async (req,res)=>{
  ...
  //Information about the staff that we would get from the req.body.id
     const staffid_dayoff = req.body.staffid
     //try{
     const memberstaff_dayoff = await staffMemberModel.findOne({id:staffid_dayoff})
     if(memberstaff_dayoff==null){
       res.send("The staffid is not in our database")
     }else{
    //An error might occur herer Take care change to req.body.id
    var department_of_memberstaff_dayoff = memberstaff_dayoff.department//}
     //catch(error){
     // console.log("The id is not in our database")
     // res.send("The id is not in our database") 
    //}
     var role_of_memberstaff_dayoff = memberstaff_dayoff.role
     }
     if(role_of_memberstaff=='HOD'){
       if(department_of_memberstaff==department_of_memberstaff_dayoff){
          
        try{
             const staff = await staffMemberModel.find({id:staffid_dayoff},'id name email dayOff')
            res.send(staff)
          }catch(error){
          console.log("There is an error")
          res.send(error)
...
router.route('/AcceptRequest')
  .post(async (req,res)=>{
    ...
    //Info of a Given A Request from post
     const reqx_given_type = req.body.reqtype
     const reqx_sender = req.body.reqsender
     const reqx_date = req.body.reqdate  //request date
     const reqx_startdate = req.body.reqx_startDate
     const reqx_enddate = req.body.reqx_endDate
     //try{
     const reqx =await requestModel.findOne({type:req_given_type,sender:reqx_sender,requestDate:reqx_date,startDate:reqx_startdate,endDate:reqx_enddate}) 
     if(reqx==null){
      res.send("Either there is no user with that id or no request with such type  ")
    } 
     
     const sendermodel = await staffMemberModel.findOne({id:reqx_sender})
     const dayoff_of_the_sender_before_accepting = sendermodel.dayOff
    const schedule_of_sender = await scheduleModel.find({academicMember:reqx_sender})
    ...
    //day that you wanna change
      ...
      try{
        if(req.body.day){
     if(reqx_given_type=='change dayoff'){
        await requestModel.updateOne({type:req_given_type,sender:reqx_sender,requestDate:reqx_date,startDate:reqx_startdate,endDate:reqx_enddate},{status:"Accepted"})    
         
        for(var i = 0;i<schedule_of_sender.length;i++){
           if((req.body.date).toDateString() === (schedule_of_sender[i].day).toDateString())
             await scheduleModel.updateOne({academicMember:reqx_sender,day:schedule_of_sender[i].day},{academicMember:null})
          
          }
           await staffMemberModel.updateOne({id:reqx_sender},{dayOff:req.body.day})
   ...
   //day that you wann change
      ...
      try{
        ...
        if(req_given_type=='change dayoff'){
           await requestModel.updateOne({type:req_given_type,sender:reqx_sender,requestDate:reqx_date,startDate:reqx_startdate,endDate:reqx_enddate},{status:Rejected})    
  ...
  .post(async (req,res) =>{
   ...
   const total = number_of_instructors+number_of_Tas+numberofcoordinator
    console.log(total)
    var coverage =  (100*number_of_slots)/total
